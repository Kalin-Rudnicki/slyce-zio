...
trait JsonDecoder[+T] { self =>
  ...
  final def <*>[T2](other: JsonDecoder[T2])(using zip: zio.Zippable[T, T2]): JsonDecoder[zip.Out] =
    json =>
...
object JsonDecoder {
  ...
  inline def apply[T](using je: JsonDecoder[T]): JsonDecoder[T] = je
  ...
  given jsonObject: JsonDecoder[Map[String, Json]] = JsonDecoder.fromPartial("object") { case Json.JsonObject(value) => value }

  given unit: JsonDecoder[Unit] = JsonDecoder.fromPartial("null") { case Json.JsonNull => () }
  given string: JsonDecoder[String] = JsonDecoder.fromPartial("string") { case Json.JsonString(value) => value }
  given boolean: JsonDecoder[Boolean] = JsonDecoder.fromPartial("boolean") { case Json.JsonBoolean(value) => value }
  given int: JsonDecoder[Int] = JsonDecoder.fromPartial("int") { case Json.JsonInt(value) => value }
  given bigDecimal: JsonDecoder[BigDecimal] = JsonDecoder.fromPartial("double") {
    case Json.JsonDouble(value) => value
    case Json.JsonInt(value)    => BigDecimal(value)
  }
  given float: JsonDecoder[Float] = JsonDecoder.bigDecimal.map(_.toFloat)
  given double: JsonDecoder[Double] = JsonDecoder.bigDecimal.map(_.toDouble)
  given array: [T: JsonDecoder] => JsonDecoder[List[T]] =
  ...
  given map: [T: JsonDecoder] => JsonDecoder[Map[String, T]] =
  ...
  given optional: [T: JsonDecoder] => JsonDecoder[Option[T]] =
